...
#CFD from original author (also refer to the CFDScoring folder)
...
nuclFreqs= {('A', 'A'): 0.4819440141370613, ('G', 'G'): 0.6571297543038187, ('U', 'T'): 0.4663759334374003,
            ('U', 'C'): 0.18755561795635842, ('C', 'T'): 0.3917125484856841, ('G', 'A'): 0.472948896301865,
            ('G', 'T'): 1.0, ('A', 'G'): 0.2796160896968995, ('U', 'G'): 0.787929779020387, ('C', 'C'): 0.0,
            ('A', 'C'): 0.6804018833297995, ('C', 'A'): 0.5931243444910334}
posFreqs = [0.294369386, 0.29164666, 0.190210984, 0.306896763, 0.167251773, 0.219909422, 0.169797251,
            0.406475982, 0.628680509, 0.588183598, 0.907111342, 0.522909141, 1.256594863, 0.693851359,
            0.552888666, 1.158572718, 1.662766602, 1.01548686, 1.428913939]
...
def main():
    ...
    # load csv file
    oseq_path = folder_path+"/OT_screening.csv"
    output_path = folder_path+"/OT_screening_function_scores.csv"
    pd_oseq= pd.read_csv(oseq_path)

    # Testing commands:
    # wt_test = "GAGTCCGAGCAGAAGAAGAA"
    # ot_test = "GAGTCCGAGCAGAAGAAGAG"
    # MMpattern="...................*"

    # Cropit = Cropitoff(wt_test,ot_test)
    # Hsu = Hsuoff(wt_test,ot_test)
    # CCTOP = CCTOPoff(MMpattern)
    # COSMID = COSMIDoff(MMpattern,1)
    # MIT = calcMitScore(wt_test,ot_test)

    #Replace NRG(from CasOffinder query) into NGG.
...
#get the info of the first base
    ...
    pd_oseq.to_csv(output_path,index=False)
